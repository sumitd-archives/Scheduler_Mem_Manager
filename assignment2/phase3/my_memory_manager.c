...
/*int main() {
    mem_manager_init();
    int * arr = (int*)myallocate(10*sizeof(int) , USER);
    int * arr1 = (int*)myallocate(20*sizeof(int) , USER);
    int * arr2 = (int*)myallocate(1000*sizeof(int) , USER);*/
    /*printf("offset = %d\n" , *(int*)(lib_book_keep_start_loc));
    printf("offset = %d\n" , *(int*)(lib_book_keep_start_loc + 4));
    mydeallocate(arr2 , LIBRARY);
    mydeallocate(arr , LIBRARY);
    printf("offset = %d\n" , *(int*)(lib_book_keep_start_loc + 4));*/
 /*   printf("offset = %d\n" , *(int*)(book_keeping_start_loc + 4));
    printf("offset = %d\n" , *(int*)(book_keeping_start_loc + 12));
    //mydeallocate(arr1 , USER);
    mydeallocate(arr , USER);
    mydeallocate(arr2 , USER);
    printf("offset = %d\n" , *(int*)(book_keeping_start_loc + 4));
    return 0;
}*/
    /*for(thread = 0 ; thread < 1790 ; thread++) {
        arr = (int*)myallocate(100*sizeof(int) , 2);
        arr[0] = thread;
        ProtectContextPages();
    }
    for(thread = 1787 ; thread < 1790 ; thread++) {
    //   UnprotectContextPages();
       char * page_loc = user_mem_pages_start_loc;
       printf("Val = %d\n" , *(int*)page_loc); 
       ProtectContextPages();
    }    
    return 0;
    ...
    int * arr = (int*)myallocate(600*sizeof(int) , 1 , 2);
    for (i = 0 ; i < 600 ; i++) 
        arr[i] = 1;
    int * arr1 = (int*)myallocate(600*sizeof(int) , 2 , 2);
    for (i = 0 ; i < 600 ; i++) 
        arr1[i] = 2;
    CopyToSwap(0 , -1);    
    CopyToSwap(1 , -1);    
    printf("swap 0 : thread = %d, page = %d, offset = %d\n " , swap_info_table[0].thread , swap_info_table[0].page , swap_info_table[0].offset);
    printf("swap 1 : thread = %d, page = %d, offset = %d\n " , swap_info_table[1].thread , swap_info_table[1].page , swap_info_table[1].offset);
    SwapOutAndFetch(0 , 1 , 2);
    SwapOutAndFetch(1 , 0 , 1);
    for (i = 0 ; i < 600 ; i++) 
        sum = sum + arr[i] ;
    for (i = 0 ; i < 600 ; i++) 
        sum1 = sum1 + arr1[i] ;
    ...
    //protect page and see
    //mprotect(user_mem_pages_start_loc , page_size , PROT_READ);
    int * arr2 = (int*)myallocate(100*sizeof(int) , 1 , 2);
    for (i = 0 ; i < 100 ; i++) {
        arr2[i] = 3;
    }
    sum = 0;
    ...
    int * arr3 = (int*)myallocate(100*sizeof(int) , 2 , 2);
    for (i = 0 ; i < 100 ; i++) {
        arr3[i] = 4;
    }
    sum = 0;
